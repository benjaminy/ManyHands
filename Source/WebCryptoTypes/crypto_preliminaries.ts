...
function makeUniqueId( ids, len : number )
{
    var id;
    var found = false;
    if( !len )
    ...
    while( !found )
    {
        id = U.bufToHex( getRandomBytes( len ) );
        if( !( id in ids ) )
            found = true;
    }
    return id;
...
function importKeyJwk( s, algo, ops, scp:scp )
{
